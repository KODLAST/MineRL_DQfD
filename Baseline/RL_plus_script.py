...
# Parameters:
config = {
    "TRAIN_TIMESTEPS": 2000000,  # number of steps to train the agent for. At 70 FPS 2m steps take about 8 hours. 2000000
    "TRAIN_ENV": 'MineRLTreechop-v0',  # training environment for the RL agent. Could use MineRLObtainDiamondDense-v0 here.
    "TRAIN_MODEL_NAME": 'train_ppo_carrot',  # name to use when saving the trained agent.
    "TEST_MODEL_NAME": '/home/drl-68/Project_6405_6440/MineRL2021-Intro-baselines/standalone/train_ppo_carrot.zip',  # name to use when loading the trained agent.
    ...
    "TREECHOP_STEPS": 2000,  # number of steps to run RL lumberjack for in evaluations. 18k
...
def plot_flat_rgb(flat_data, image_size=(64, 64)):
    global fig, ax, im

    # Reshape the flat RGB array into an image
    image = flat_data.reshape((*image_size, 3)).astype(np.uint8)
