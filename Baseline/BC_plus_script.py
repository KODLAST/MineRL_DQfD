...
# Parameters:
DATA_DIR = "/home/drl-68/Project_6405_6440/MineRL2021-Intro-baselines/data"  # path to where MineRL dataset resides (should contain "MineRLTreechop-v0" directory).
EPOCHS = 3  # How many times we train over the dataset.
LEARNING_RATE = 0.0001  # Learning rate for the neural network.

TRAIN_MODEL_NAME = 'test_BC_carrot3.pth'  # name to use when saving the trained agent.
TEST_MODEL_NAME = '/home/drl-68/Project_6405_6440/MineRL2021-Intro-baselines/standalone/test_BC_carrot3.pth'  # name to use when loading the trained agent.

TEST_EPISODES = 9  # number of episodes to test the agent for.
...
SAVE_FILE_PATH = "result/BC_LOSS.json"
REWARD_FILE_PATH = "result/BC_REWARD.json"
ACTION_SAVE_PATH = "result/BC_ACTION.json"

# Initialize global figure and axis
...
def test():
    ...
    for episode in range(TEST_EPISODES):
        ...
        for i in range(TREECHOP_STEPS):
            ...
            actionsList.append( action.item() )
            # print( 'prob ',probabilities, 'action :', action )
    ...
    saveJson( ACTION_SAVE_PATH, actionsList )
    saveJson( REWARD_FILE_PATH, reward_every_episode )
    # with open(ACTION_SAVE_PATH, 'w') as f:
    #     json.dump(actionsList, f, indent=4)

    # with open(REWARD_FILE_PATH, 'w') as f:
    #     json.dump(reward_every_episode, f, indent=4)

    env.close()

def saveJson( FILEPATH, list ):
